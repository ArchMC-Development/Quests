...
import lol.arch.quests.models.Dimension;
...
public class QuestTrackingListener implements Listener {
    ...
    private <T> void handleQuestProgressIncrease(
        ...
        ShortTermDataService.INSTANCE.byId(uuid)
                .thenAccept(profile -> {
                    if (profile == null) {
                        profile = new ShortTermQuestProfile(uuid, new HashSet<>(), "", 0);
                    }

                    String questId = profile.getCurrentQuestId();
                    Quest quest = ActiveQuestsDataSync.INSTANCE.cached().getCache().get(UUID.fromString(questId));
                    if (quest == null) {
                        return;
                    }
                    ...
                    var questCache = ActiveQuestsDataSync.INSTANCE.cached().getActive();

                    int requirement = questCache.get(UUID.fromString(questId));

                    profile.incrementQuestProgress(progressIncrease);
                    displayBossBarProgress(player, profile, quest, requirement);

                    // Quest completion
                    int progress = profile.getCurrentQuestProgress();
                    if (progress >= requirement) {
                        ...
                        profile.setCurrentQuestIdAsCompleted();
                        Tasks.sync(() -> {
                        ...
                        if (profile.getDailyCompletedQuests().size()
                                >= questCache.size()) {
                    ...
                    player.sendActionBar(
                            UtilChat.colorize(
                                    questActionBar
                                            .replaceAll("%1%", quest.getDisplayName())
     ...
     */
    private void displayBossBarProgress(Player player, ShortTermQuestProfile playerData, Quest quest, int requirement) {
        UUID uuid = player.getUniqueId();
                ...
                Bukkit.createBossBar(
                        UtilChat.colorize(
                                questsConfig
                                        .getLang(player)
                                        .getQuestBossBar()
                                        .replace("%1%", quest.getDisplayName())
                                        .replace("%2%", playerData.getCurrentQuestProgress() + "")
                                        .replace("%3%", requirement + "")),
                        BarColor.GREEN,
                        BarStyle.SOLID);
